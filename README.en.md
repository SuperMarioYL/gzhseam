...
- **v0.5.0** — wash config-override fix: `wash()` previously ignored the documented `GzhCtx.whitelisted_tags` / `allowed_attrs` / `allowed_fonts` per-call overrides — stages 1/2 passed only the html string to `whitelist.filter_html` and `fonts.normalize_fonts`, neither of which accepted an override parameter, so the module-level defaults were always used. `filter_html` and `normalize_fonts` now take optional override params (defaulting to the existing module constants) and `wash()` threads the ctx fields through; default behavior and existing call signatures are unchanged, but a caller building `GzhCtx(whitelisted_tags=stricter_set)` (e.g. an enterprise no-<a>/no-<img> policy), a relaxed set, or a custom font whitelist is honored instead of silently swallowed.
